...
contract RevenueRouterTest is Test {
    ...
    function testRoutesCashAndCompoundsExternalRevenue() public {
        RevenueRouter.CashRecipient[] memory cash =
            new RevenueRouter.CashRecipient[](1);
        cash[0] = RevenueRouter.CashRecipient({
            account: cashRecipient,
            shareBps: 4_000
        });

        RevenueRouter.CompoundRecipient[] memory compound =
            new RevenueRouter.CompoundRecipient[](1);
        compound[0] = RevenueRouter.CompoundRecipient({
            vault: IResearchVault(address(vault)),
