# Source register

As-of date: **2026-09-03**. URLs are included for repository maintainers; re-check current versions before production implementation.

## OpenAI / ASTRA

- GPT-6 Astra model documentation: https://developers.openai.com/api/docs/models/gpt-6-astra
- Current model guidance and Responses API usage: https://developers.openai.com/api/docs/guides/latest-model
- Responses API migration and object model: https://developers.openai.com/api/docs/guides/migrate-to-responses
- Reasoning model guidance: https://developers.openai.com/api/docs/guides/reasoning

The ASTRA adapter keeps the model name configurable and records model/version data in compute receipts because availability, pricing, and capabilities can change.

## x402

- Payment schemes overview: https://docs.x402.org/schemes/overview
- HTTP 402 flow: https://docs.x402.org/core-concepts/http-402
- Facilitator responsibilities: https://docs.x402.org/core-concepts/facilitator
- Exact scheme: https://docs.x402.org/schemes/exact
- Upto scheme: https://docs.x402.org/schemes/upto
- Batch settlement: https://docs.x402.org/schemes/batch-settlement
- Payment identifier/idempotency: https://docs.x402.org/extensions/payment-identifier
- Extensions overview: https://docs.x402.org/extensions/overview

## Lean

- Lean home and documentation: https://lean-lang.org/
- Validating a Lean proof: https://lean-lang.org/doc/reference/latest/ValidatingProofs/
- Lean package registry: https://reservoir.lean-lang.org/
- Verso documentation system: https://verso.lean-lang.org/
- leanblueprint: https://github.com/PatrickMassot/leanblueprint

The verification architecture follows the official distinction between ordinary builds, axiom inspection, fresh checking, and high-risk comparator/external-checker workflows.

## Ethereum standards

- ERC-1155 Multi Token Standard: https://eips.ethereum.org/EIPS/eip-1155
- ERC-4626 Tokenized Vaults: https://eips.ethereum.org/EIPS/eip-4626
- EIP-712 typed structured data: https://eips.ethereum.org/EIPS/eip-712
- ERC-7540 asynchronous ERC-4626 flows: https://eips.ethereum.org/EIPS/eip-7540

## Canonicalization and content addressing

- RFC 8785 JSON Canonicalization Scheme: https://www.rfc-editor.org/rfc/rfc8785
- IPFS content addressing: https://docs.ipfs.tech/concepts/content-addressing/

## Consensus and committee selection

- HotStuff: BFT consensus with linear communication: https://arxiv.org/abs/1803.05069
- Generalized Byzantine quorums: https://arxiv.org/abs/2006.04616
- Algorand cryptographic sortition dissertation/paper materials: https://dspace.mit.edu/handle/1721.1/129446

## Rights and token boundaries

- U.S. Copyright Office, Copyright and Artificial Intelligence: https://www.copyright.gov/ai/
- U.S. Copyright Act §102: https://www.copyright.gov/title17/92chap1.html#102
- SEC digital-asset investment-contract framework archive/current materials: https://www.sec.gov/corpfin/framework-investment-contract-analysis-digital-assets
- Molecule IP-NFT materials as prior art: https://molecule.xyz/
- Proofgold prior art: https://drops.dagstuhl.de/entities/document/10.4230/OASIcs.FMBC.2022.4

These sources inform design boundaries only. Jurisdiction-specific counsel is required before public token, licensing, vault, investment, or payment deployment.
